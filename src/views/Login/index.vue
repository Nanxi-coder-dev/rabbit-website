...
//表单校验（账号+密码）
//思想：当功能很复杂时，通过多个组件各自负责某个小功能，再合成一个大功能 
...
const rules = reactive({
    ...
    password: [
        ...
        {min: 6, max: 14, message: "密码长度为6-14个字符", tigger: 'blur'}
    ],
    agree: [
        {
          validator: (rule, value, callback) => {
            //自定义校验逻辑：勾选协议通过，不勾选协议就不通过
            if(value){
              //通过时直接调用函数
              callback()
            } else{
              //不通过时也调用函数，并new Error提示
